package bitcoinlib_test

import (
	"bitcoinlib/bitcoinlib"
	"bytes"
	"encoding/hex"
	"testing"
)

func TestVersionParsing(t *testing.T) {
	version_buf := bytes.NewReader([]byte{0x01, 0x00, 0x00, 0x00, 0xff})
	result, err := bitcoinlib.NewVersionFrom(version_buf)
	if err != nil {
		t.Fatalf("Faile with error %s", err)
	}
	expected := bitcoinlib.NewVersion(1)
	if expected.Ne(*result) {
		t.Fatalf("%v is different from %v", result, expected)
	}
}

func TestInputOutputParsing(t *testing.T) {
  transaction, _ := hex.DecodeString("")
  parsed, err := bitcoinlib.ParseTransaction(bytes.NewReader(transaction))
  if err != nil {
    t.Fatalf("Failed at parsing with: %s", err)
  }
  expectedInput := "304402207899531a52d59a6de200179928ca900254a36b8dff8bb75f5f5d71b1cdc26125022008b422690b8461cb52c3cc30330b23d574351872b7c361e9aae3649071c1a71601 035d5c93d9ac96881f19ba1f686f15f009ded7c62efe85a872e6a19b43c15a2937"
  inputs := parsed.GetInputs()
  outputs := parsed.GetOutputs()
  if expectedInput != inputs[1] {
    t.Fatalf("Failed at inputs: %s\n!=\n%s\n%d", outputs[0], inputs[1], parsed.GetOutputsAmount()[1])
  }
}
